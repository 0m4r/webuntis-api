  ...
  try {
    ...
    if (todayLessons.length > 0) {
      ...
      todayLessons.forEach((lesson, index) => {
        ...
        const subjects = lesson.su.map((s: { name: string }) => s.name).join(", ") || "Unknown";
        const teachers = lesson.te.map((t: { name: string }) => t.name).join(", ") || "Unknown";
        const rooms = lesson.ro.map((r: { name: string }) => r.name).join(", ") || "Unknown";
    ...
    console.log(
      `📊 Found ${weekLessons.length} lessons for this week (${formatDate(weekStart)} - ${formatDate(weekEnd)})`,
    );
    ...
    try {
      ...
      if (homeworks.length > 0) {
        console.log("\n📝 Upcoming Homework:");
        homeworks?.slice(0, 5).forEach((hw: Homework, index: number) => {
...
async function demonstrateSecretAuth(): Promise<void> {
  ...
  try {
    ...
    if (classes.length > 0) {
      ...
      const today = new Date();
      const classTimetable = await untis.getTimetableForToday(sampleClass.id, WebUntisElementType.CLASS);

      console.log(`📊 Found ${classTimetable.length} lessons for ${sampleClass.name} today`);

      if (classTimetable.length > 0) {
        console.log("\n📅 Class Schedule:");
        classTimetable.slice(0, 3).forEach((lesson: Lesson, index: number) => {
          const startTime = formatTime(lesson.startTime);
          const endTime = formatTime(lesson.endTime);
          const subjects = lesson.su.map((s: { name: string }) => s.name).join(", ");
          const teachers = lesson.te.map((t: { name: string }) => t.name).join(", ");
