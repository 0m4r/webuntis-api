 ...
 *
 * Usage:
 *   WEBUNTIS_SCHOOL=... WEBUNTIS_USERNAME=... WEBUNTIS_PASSWORD=... node --loader ts-node/esm apps/examples/src/runtime-type-check.ts
 *   # or bundle first (yarn nx run examples:build) and run node apps/examples/dist/runtime-type-check.js
 */
import "dotenv/config";
import { WebUntis } from "webuntis-api/src/index.ts";
import { z } from "zod";
import {
  schoolYearSchema,
  newsWidgetSchema,
  ...
  homeworkSchema,
  homeworksSchema,
} from "../../../packages/webuntis-api/src/types.schema.ts";
...
function readEnv(): EnvConfig {
  ...
  const missing = ["WEBUNTIS_SCHOOL", "WEBUNTIS_USERNAME", "WEBUNTIS_PASSWORD"].filter((k) => !process.env[k]);
  if (missing.length) {
...
async function main(): Promise<void> {
  ...
  const checks: Array<{
    ...
    },
    {
      ...
      skip: !hasPerson,
      run: () => untis.getPdfOfAbsentLesson(today, nextWeek, true, -1, true, true, 2),
    },
  ];
