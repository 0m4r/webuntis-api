...
class TimetableAnalyzer {
    ...
    static generateReport(analysis: TimetableAnalysis): string {
        ...
        const sortedSubjects = Array.from(analysis.subjectDistribution.entries())
            .sort((a, b) => b[1] - a[1]);
        sortedSubjects.slice(0, 10).forEach(([subject, count]) => {
        ...
        // Peak hours
        report += '\nPeak Hours:\n';
        analysis.peakHours.slice(0, 5).forEach(({ hour, lessonCount }) => {
...
async function demonstrateBulkOperations(untis: WebUntis): Promise<void> {
    ...
    try {
        ...
        Logger.success(`Processed ${results.length} classes, ${totalLessons} total lessons, avg ${Math.round(avgDuration)}ms per class`);
