...
This directory contains comprehensive examples demonstrating how to use the WebUntis API effectively. The examples progress from basic usage to advanced, production-ready implementations.
...
1. **Configure Your Credentials**
    ...
    Edit `config.ts` and replace the placeholder values with your actual WebUntis credentials:
...
# or with ts-node
npx ts-node examples/01-basic-example.ts
```
...
```bash
node examples/02-intermediate-example.js
# or with ts-node
npx ts-node examples/02-intermediate-example.ts
```
...
```bash
node examples/03-comprehensive-example.js
# or with ts-node
npx ts-node examples/03-comprehensive-example.ts
```
...
```bash
DEBUG=webuntis* node examples/01-basic-example.js
```
...
For complete API documentation, visit: [https://webuntis.noim.me/](https://webuntis.noim.me/)
...
1. Check the [official documentation](https://webuntis.noim.me/)
