...
export class Base {
   ...
   * @returns {Promise.<Array>}
   */
  async getOwnTimetableForRange(rangeStart: Date, rangeEnd: Date, validateSession: boolean = true): Promise<Lesson[]> {
    this._checkAnonymous();
   ...
   */
  async getHomeWorkAndLessons(rangeStart: Date, rangeEnd: Date, validateSession: boolean = true): Promise<Array<any>> {
    ...
    const formatElements = (elements: Array<Record<string, unknown>>, { byType }: { byType: number }) => {
      ...
      return filteredElements.map((element) => ({
        ...
        element: data.elements.find(
          (dataElement: Record<string, unknown>) => dataElement.type === byType && dataElement.id === element.id,
        ),
      }));
    };
...
export class InternalWebuntisSecretLogin extends Base {
  ...
  async _otpLogin(
    ...
    if (responseData && responseData.error) throw new Error("Failed to login. " + (responseData.error.message || ""));
